...
import {
    getFeedbackTemplateList,
    } from '../../../../src/api/SystemManagement/ReviewTemplateConfig';
...
export default {
    ...
    data () {
        return {
            ...
            // 表格数据
            tableKey: ['平台账号', '纠纷ID', '纠纷类型', 'Case ID', '操作类型', '订单号', '退款状态', '售后单号', '操作时间', '售后单创建时间'],
            tableProps: ['account_name', 'dispute_id', 'dispute_type', 'case_id', 'operate_type', 'order_id', 'refund_status', 'after_sale_id', 'operate_time', 'after_sale_time'],
            tableWidth: ['', '', '', '', '', '', '', '', '', ''],
            tableData: [],
            // 表格加载动画
            tableLoading: false,
    ...
    methods: {
        ...
        handleQueryInfo(params, isSave, isComSearch) {
            ...
            // 判断是否取缓存参数
            if (isSave) {
                this.queryData = JSON.parse(this.queryParamsInfo['review_template_config']);
            }
            let obj = {};
            // 分离头部搜索和快捷搜索两种条件
            if( !isComSearch ){
