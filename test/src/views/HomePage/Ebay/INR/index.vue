...
<template>
    <div class="EbayINRwrap">
        <div class="EbayINRbox">
            ...
            <div class="ui-main-module">
                ...
                <ComTableInfo
                    ...
                    <template slot-scope="scope" slot="accountinfo">
                        <div>Inquiry ID：{{scope.row.inquiry_id}}</div>
                    ...
                    <template slot-scope="scope" slot="disputeinfo">
                        ...
                        <div>买家期望：{{scope.row.buyer_initial_expected_resolution}}</div>
                        <div>买家最初期望退款金额：<span style="color: #f90; font-weight: bold;">{{scope.row.buyer_init_expect_refund_amt}} ({{scope.row.currency}})</span></div>
                        <div>卖家发放退款金额：<span style="color: #f90; font-weight: bold;">{{scope.row.refund_amount}} ({{scope.row.currency}})</span></div>
                    ...
                    <template slot-scope="scope" slot="timeinfo">
                        ...
                        <div>最新收信时间：{{scope.row.last_buyer_respdate}}</div>
                        <div>创建时间：{{scope.row.creation_date}}</div>
                        <div>更新时间：{{scope.row.update_time}}</div>
                        <div>截止时间：{{scope.row.expiration_date}}</div>
                        <div>发货时间：{{scope.row.order_data && scope.row.order_data.shipped_date}}</div>
                        <div>预计妥投时间：{{scope.row.scheduled_delivery_start_date}}</div>
                    </template>
                    ...
                    <template slot-scope="scope" slot="operate">
                        <el-button v-if="buttonList['dealWith'].permit(thisVm)" @click="handlePageINR(scope.row)" type="text" size="mini">处理</el-button>
                        <ComDivide v-if="buttonList['dealWith'].permit(thisVm)"/>
                        <el-button v-if="buttonList['log'].permit(thisVm)" @click="handleLogFn(scope.row.inquiry_id)" type="text" size="mini">日志</el-button>
                    </template>
                </ComTableInfo>
...
export default {
    ...
    methods: {
        ...
        handleQueryInfo(params, isSave, isComSearch) {
            ...
            // 判断是否取缓存参数
            if (isSave) {
                this.queryData = JSON.parse(this.queryParamsInfo['Ebay_INR']);
            }
            let obj = {
        ...
        batchKefusubmitFn() {
            this.$refs.batchKefuForm.validate((valid) => {
                if (valid) {
                    let id = this.saveCheckInfo.map(e => e.inquiry_id);
        ...
        dataDownloadFn() {
            this.$confirm('是否导出数据？', '提示', {
                ...
                let data = JSON.parse(this.queryParamsInfo['Ebay_INR'] || '{}');
                let datastr = {};
        ...
        handleOrderInfo(row){
            this.orderInfo={
                order_id:row.order_data && row.order_data.order_id,
                platform_code:'EB',
                edit:false,
                title:'订单信息',
