            ...
            <div class="ui-main-module">
                ...
                <ComTableInfo
                    ...
                    <template slot-scope="scope" slot="email_cate">
                        ...
                        <div v-if="scope.row.type_value">{{scope.row.type_value}}</div>
                        <div>{{scope.row.mail_category}}</div>
                    </template>
                    <template slot-scope="scope" slot="payment_status">
                        <div>支付状态：
                            <span class="ui-state_circle orangeColor" v-if="scope.row.payment_status === '未支付'"></span>
                            <span class="ui-state_circle blueColor" v-if="scope.row.payment_status === '已支付'"></span>
                            {{scope.row.payment_status}}
                        </div>
                        <div>发货状态：
                            <span class="ui-state_circle orangeColor" v-if="scope.row.complete_status === '未发货'"></span>
                            <span class="ui-state_circle blueColor" v-if="scope.row.complete_status === '已发货'"></span>
                            {{scope.row.complete_status}}
                        </div>
                    </template>
                    <template slot-scope="scope" slot="receive_date">
        ...
        <el-dialog
            ...
            <div>
                <el-form :model="batchEditObj.batchSortForm" :rules="formRules" ref="batchSortForm" size="small" label-width="100px">
                    <el-form-item prop="three" label="模块分类：">
                        <el-select v-model="batchEditObj.batchSortForm.three">
                            <el-option
                                v-for="(v, i) in [{id: 1, name: '消息中心'}, {id: 2, name: '纠纷中心'}]"
                                :key="i"
                                :label="v.name"
                                :value="v.id">
                            </el-option>
...
export default {
    ...
    data () {
        return {
            ...
            tableWidth: ['86', 'subject', 'sender', '', '', '240', '120', '70', '130', '210', '130'],
            tableData: [],
            // 表格加载动画
            tableLoading: false,
    ...
    methods: {
        ...
        handleQueryInfo(params, isSave, isComSearch) {
            ...
            this.GetInboxList(obj).then(({data}) => {
        ...
        beforeGoMailProcessingPage(arr, curParams) {
            ...
            let getDesInfo = (obj, originObj) => {
                // 处理需要发送的搜索参数
                let data = JSON.parse(this.queryParamsInfo['in_box'] || '{}');
                let datastr = {};
                ...
                return {
                    ...
                    input_inbox: JSON.stringify(datastr),
                };
            };
        ...
        batchTagsubmitFn() {
            this.$refs.batchTagForm.validate((valid) => {
                if (valid) {
                    ...
                    addTag({buyer_email: id, tag_id: this.batchEditObj.batchTagForm.tag, platform_code: this.platform}).then(({data}) => {
                        if (data.status !== 1) {
                            this.$message.error(data.errorMess || '操作失败')
                            return;
                        }
        ...
        batchForward() {
            ...
            let path = '/PostMail';
            let query = {id: this.saveCheckInfo.map(e => e.id).join(','), type: 'Inbox', goBackUrl: JSON.stringify({path: this.$route.path, query: this.$route.query})};
