...
export default {
    ...
    data () {
        return {
            ...
            // 表格数据
            tableKey: ['销售账号', '订单号', '发送状态', '标题', '客户ID/邮箱', '发件邮箱', '发送人', '发送时间', '操作'],
            tableProps: ['account_name', 'order_id', 'send_status', 'subject', 'receive_name', 'sender_name', 'sender', 'send_time', 'operate'],
            tableWidth: ['120', '240', '120', 'subject', 'receive_name', 'sender_name', 'sender', '210', '150'],
            tableData: [],
            // 表格加载动画
            tableLoading: false,
    ...
    mounted() {
        try{
            if (this.$route.query._cacheType === '1' || JSON.stringify(this.OutboxObj) === '{}') {
                ...
                // 获取保存的查询参数
                this.queryData = JSON.parse(this.queryParamsInfo['out_box']);
    ...
    methods: {
        ...
        handleQueryInfo(params, isSave, isComSearch) {
            ...
            this.GetOutboxList(obj).then(({data}) => {
        ...
        beforeGoMailProcessingPage(arr, curParams) {
            ...
            let getDesInfo = (obj, originObj) => {
                // 处理需要发送的搜索参数
                let data = JSON.parse(this.queryParamsInfo['out_box'] || '{}');
                let datastr = {};
                ...
                return {
                    ...
                    point: 'outbox',
                    type: 'Outbox',
                    out_box: obj.inbox_id,
                    email_cate_id: originObj.email_cate_id,
                    input_outbox: JSON.stringify(datastr)
                };
            };
        ...
        batchForward() {
            ...
            let path = '/PostMail';
            let query = {id: this.saveCheckInfo.map(e => e.id).join(','), type: 'Outbox', goBackUrl: JSON.stringify({path: this.$route.path, query: this.$route.query})};
        ...
        handelDelItemFn() {
            ...
            this.$confirm('确认删除?', '提示', {
                ...
                delOutboxList({id: ids}).then(({data}) => {
                    if (data.status !== 1) {
                        this.$message.error(data.errorMess || '操作失败')
                        return;
                    }
