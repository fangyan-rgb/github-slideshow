                            ...
                            <el-dropdown-menu slot="dropdown">
                                ...
                                <el-dropdown-item>
                                    ...
                                    v-if="buttonList['messageContact'].permit(thisVm)&& scope.row.return_typesmall_name!=='疑似丢件'"
                                        @click="handleMessageContact(scope.row,true)">邮件联系</el-button>
                                </el-dropdown-item>
                                <!-- 驳回 -->
                                <el-dropdown-item>
...
import {
    getReturnGoodsHomeList,
    amazonReturnGoodsHomeLog,
    amazonReturnGoodsHomeReject,
    amazonReturnGoodsHomeHangup,
} from '../../../api/orderInfo/AmazonDomesticRefund';
import {
    withhold, //暂扣
    toVoidInfo, //作废
    batchCancelWithhold,
...
export default {
    ...
    methods: {
        ...
        handleQueryInfo(params, isSave, isComSearch) {
            ...
            getReturnGoodsHomeList(obj).then(({data}) => {
        ...
        handleMessageContact(row,bool){
            let path = '/PostMail';
            let query = {type: "AmazonDomesticRefund", order_id: row.order_id, acct_email: row.account_email, email: row.buyer_email, goBackUrl: JSON.stringify({path: this.$route.path, query: this.$route.query})};
        ...
        handleOrderLog(id){
            ...
            amazonReturnGoodsHomeLog(obj).then(({data}) => {
                if (data.status !== 1) {
        ...
        handleRejectConfirm(){
            amazonReturnGoodsHomeReject(this.reject).then(({data}) => {
                if (data.status) {
                    this.$message.success('操作成功！');
        ...
        TemporarilyHandle(row){
            this.$confirm('确定暂不处理该退件？', '暂不处理', {
                ...
                amazonReturnGoodsHomeHangup({id:[row.id]}).then(({data}) => {
                    if (data.status) {
                        this.$message.success('操作成功！');
                        this.handleQueryInfo({}, true);
                    } else {
        ...
        //评价跳转
        goFeedBackjump(row){
            this.$router.push({path:'/CustomerReviewsFeedBack',query:{type:"AmazonDomesticRefund",order_id: row.order_id, _cacheType:'1'}});
