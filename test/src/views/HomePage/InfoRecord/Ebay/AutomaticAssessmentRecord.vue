            ...
            <div class="ui-main-module">
                ...
                <ComTableInfo
                    ...
                    <template slot-scope="scope" slot="status_name">
                        <span class="ui-state_circle grayColor" v-if="scope.row.status_name === '等待发送'"></span>
                        <span class="ui-state_circle greenColor" v-if="scope.row.status_name === '发送成功'"></span>
                        <span class="ui-state_circle redColor" v-if="scope.row.status_name === '发送失败'"></span>
                        <span class="ui-state_circle blueColor" v-if="scope.row.status_name === '发送中'"></span>
...
import {getListEbay,getLogListEbay} from '../../../../api/InfoRecord/Ebay/AutomaticAssessmentRecord';

export default {
    name: 'AutomaticAssessmentRecord',
    data () {
        return {
            thisVm: this,
            showSearch: true,
            ...
            // 表格数据
            tableKey: ['序号', '规则名称', '平台账号', '订单号', 'Item ID', '关联模板', '留评状态', '留评时间', '留评失败原因', '操作'],
            tableProps: ['index', 'rule_name', 'account_name', 'order_id', 'item_id', 'template_name', 'status_name', 'create_time', 'error', 'operate'],
            tableWidth: ['50', '150', '120', '120', '120', '120', '100', '180', '', '100'],
            tableData: [],
            // 表格加载动画
            tableLoading: false,
    ...
    methods: {
        ...
        //日志弹窗
        handleJournal(row){
            getLogListEbay({id:row.order_id}).then(({data}) => {
                if (data.status !== 1) {
